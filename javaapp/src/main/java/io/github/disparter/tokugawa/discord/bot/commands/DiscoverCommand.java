...
public class DiscoverCommand implements SlashCommand {
    ...
    private Mono<Void> showHiddenAreas(ChatInputInteractionEvent event, Player player) {
    ...
    private Mono<Void> discoverArea(ChatInputInteractionEvent event, Player player, String areaName) {
        Location currentLocation = locationService.getPlayerLocation(player.getId());
